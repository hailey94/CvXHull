...
def get_layer_output_sizes(model, data, mode=None, pad_length=constants.PAD_LENGTH):   
    output_sizes = {}
    hooks = []
    name_counter = {}
    ...
    def hook(module, input, output):
        ...
        if mode == 'regression':
            output_sizes['%s-%d' % (class_name, name_counter[class_name])] = list(output.size())
        else:
...
def get_layer_output(model, data, pad_length=constants.PAD_LENGTH, get_final=False, get_max=False):
    with torch.no_grad():
        name_counter = {}        
        layer_output_dict = {}
        layer_dict = get_model_layers(model)
